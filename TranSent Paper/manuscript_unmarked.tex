...
The proposed TranSenti, as illustrated in \hyperref[fig:overall_framework]{Fig.~\ref{fig:overall_framework}}, is designed to extract insights into transit satisfaction from unstructured social media data. The initial phase, termed as \textit{Data Collection & Preprocessing}, involves gathering raw data from Weibo (and similar social media platforms) and subsequently cleaning them by removing unrelated elements, such as URLs and hashtags. The core of the framework comprises several sequential phases: Firstly, we accept the preprocessed social media data and implements a hybrid filtering process. It involves an ensemble of Large Language Models (LLMs) fine-tuned with Kullback-Leibler (KL) divergence constraints and prompted with progressively improved directives. The objective is to accurately distinguish and identify posts that actually reflect experiences with transit services from those containing irrelevant references. The output of this phase is a collection of filtered Transit-Related Posts.
...
\subsubsection{Instruction-guided passenger post filtering}
...
\subsection{Spatio-temporal information embedding for sentiment classification}\label{sec:SatisfMethod}
...
\subsubsection{Spatio-temporal encoding}
...
\begin{figure}[htbp]
    ...
    \caption{Distribution of confusion matrix components for the proposed filtering method on evaluation samples.}
...
Based on the dataset we filtered out, we then use our fine-tuned RoBERTa model to classify the sentiments of passengers' social media posts. Each of the 116,506 posts identified as being related to transit is classified as either a passenger negative comment or a passenger positive comment on the transit service. The geospatial distribution of the negative and positive sentiments is illustrated in \hyperref[fig:shenzhen_metro_combined_negative_sentiment_unified_normalized_heatmap]{Fig.~\ref{fig:shenzhen_metro_combined_negative_sentiment_unified_normalized_heatmap}} and \hyperref[fig:shenzhen_metro_combined_positive_sentiment_unified_normalized_heatmap]{Fig.~\ref{fig:shenzhen_metro_combined_positive_sentiment_unified_normalized_heatmap}}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figs/shenzhen_metro_combined_negative_sentiment_unified_normalized_heatmap.png}
    \caption{Distribution of confusion matrix components for passenger sentiment classification.}
    \label{fig:shenzhen_metro_combined_negative_sentiment_unified_normalized_heatmap}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{figs/shenzhen_metro_combined_positive_sentiment_unified_normalized_heatmap.png}
    \caption{Distribution of confusion matrix components for passenger sentiment classification.}\label{fig:shenzhen_metro_combined_positive_sentiment_unified_normalized_heatmap}
\end{figure}

\hyperref[fig:confusion_matrix_donut_new]{Fig.~\ref{fig:confusion_matrix_donut_new}} shows the distribution of confusion matrix components for the classification results. 

To assess the actual performance of our sentiment model against several baselines, we use 497 truly transit-related posts from the evaluation dataset of 1,000 samples. The numbers of true positives (i.e., transit-related posts correctly classified as passenger positive comments), true negatives (i.e., posts correctly classified as passenger negative comments on transit service), false positives, and false negatives are then counted and visualized in \hyperref[fig:confusion_matrix_donut_new]{Fig.~\ref{fig:confusion_matrix_donut_new}}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.4\textwidth]{figs/confusion_matrix_donut_new.pdf}
    \caption{Distribution of confusion matrix components for passenger sentiment classification.}
    \label{fig:confusion_matrix_donut_new}
\end{figure}
